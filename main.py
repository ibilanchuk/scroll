...
def get_module():
    result = questionary.select(
        ...
        choices=[
            ...
            Choice("24) Mint Scroll Origins NFT", nft_origins),
            Choice("25) Dmail send email", send_mail),
            Choice("26) Create gnosis safe", create_safe),
            Choice("27) Deploy contract", deploy_contract),
            Choice("28) Swap tokens to ETH", swap_tokens),
            Choice("29) Use MultiSwap", swap_multiswap),
            Choice("30) Use MultiBridge", multibridge),
            Choice("31) Use custom routes", custom_routes),
            Choice("32) Make transfer", make_transfer),
